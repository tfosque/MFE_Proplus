...
exports.createBuildTasks = exports.createBuildContext = void 0;
const path_1 = __importDefault(require("path"));
const browserslist_to_esbuild_1 = __importDefault(require("browserslist-to-esbuild"));
...
const createBuildContext = async ({ cwd, extMap, logger, pkg, }) => {
    const targets = {
        '*': (0, browserslist_to_esbuild_1.default)(pkg.browserslist ?? DEFAULT_BROWSERS_LIST_CONFIG),
        node: (0, browserslist_to_esbuild_1.default)(['node 16.0.0']),
        web: ['esnext'],
    };
    const exportsArray = (0, pkg_1.parseExports)({ extMap, pkg }).reduce((acc, x) => {
    ...
    const outputPaths = Object.values(exportsArray)
        .flatMap((exportEntry) => {
        return [exportEntry.import, exportEntry.require].filter((v) => Boolean(v));
    })
        .map((p) => path_1.default.resolve(cwd, p));
    const distPath = findCommonDirPath(outputPaths);
    if (distPath === cwd) {
        throw new Error('all output files must share a common parent directory which is not the root package directory');
    }
    if (!distPath) {
...
const pathContains = (containerPath, itemPath) => {
    return !path_1.default.relative(containerPath, itemPath).startsWith('..');
};
...
const createBuildTasks = async (ctx) => {
    ...
    const createViteTask = (format, runtime, { output, ...restEntry }) => {
        ...
        if (viteTasks[buildId]) {
            ...
            if (output !== viteTasks[buildId].output) {
                ctx.logger.warn('Multiple entries with different outputs for the same format are not supported. The first output will be used.');
            }
        }
        else {
            viteTasks[buildId] = {
    ...
    for (const exp of exps) {
        if (exp.types) {
            const importId = path_1.default.join(ctx.pkg.name, exp._path);
