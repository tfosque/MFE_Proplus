...
const disable = (message) => {
    ...
    ee.licenseInfo = (0, fp_1.pick)('licenseKey', ee.licenseInfo);
    ee.enabled = false;
    if (shouldEmitEvent) {
...
const init = (licenseDir, logger) => {
    ...
    try {
        const license = process.env.STRAPI_LICENSE || (0, license_1.readLicense)(licenseDir);
        if (license) {
            ee.licenseInfo = (0, license_1.verifyLicense)(license);
            enable();
        }
    }
    catch (error) {
...
const onlineUpdate = async ({ strapi }) => {
    ...
    try {
        ...
        const fallback = (error) => {
            if (error instanceof license_1.LicenseCheckError && error.shouldFallback && storedInfo?.license) {
        ...
        const license = shouldContactRegistry
            ? await (0, license_1.fetchLicense)({ strapi }, ee.licenseInfo.licenseKey, strapi.config.get('uuid')).catch(fallback)
            : storedInfo.license;
        if (license) {
            try {
                // Verify license and check if its info changed
                const newLicenseInfo = (0, license_1.verifyLicense)(license);
                const licenseInfoChanged = !(0, fp_1.isEqual)(newLicenseInfo.features, ee.licenseInfo.features) ||
...
const checkLicense = async ({ strapi }) => {
    const shouldStayOffline = ee.licenseInfo.type === 'gold' &&
    ...
    if (!shouldStayOffline) {
        ...
        strapi.cron.add({ [(0, cron_1.shiftCronExpression)('0 0 */12 * * *')]: onlineUpdate });
    }
    else {
...
const list = () => {
    return (ee.licenseInfo.features?.map((feature) => typeof feature === 'object' ? feature : { name: feature }) || []);
};
