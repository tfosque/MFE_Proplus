...
const upload_files_1 = __importDefault(require("../utils/upload-files"));
const components_1 = require("./components");
const params_1 = require("./params");
const attributes_1 = require("./attributes");
const { transformParamsToQuery } = utils_1.convertQueryParams;
...
const databaseErrorsToTransform = [
    database_1.errors.InvalidTimeError,
    database_1.errors.InvalidDateTimeError,
    database_1.errors.InvalidDateError,
    database_1.errors.InvalidRelationError,
];
const creationPipeline = (data, context) => {
    return (0, attributes_1.applyTransforms)(data, context);
};
...
const createDefaultImplementation = ({ strapi, db, eventHub, entityValidator, }) => ({
     ...
     */
    uploadFiles: upload_files_1.default,
...
exports.default = (ctx) => {
    ...
    const delegator = (0, delegates_1.default)(service, 'implementation');
    // delegate every method in implementation
    Object.keys(service.implementation).forEach((key) => delegator.method(key));
    // wrap methods to handle Database Errors
    service.decorate((oldService) => {
        const newService = lodash_1.default.mapValues(oldService, (method, methodName) => async function (...args) {
            try {
                ...
                if (databaseErrorsToTransform.some((errorToTransform) => error instanceof errorToTransform)) {
                    if (error instanceof Error) {
                        throw new utils_1.errors.ValidationError(error.message);
                    }
                    throw error;
                }
                throw error;
